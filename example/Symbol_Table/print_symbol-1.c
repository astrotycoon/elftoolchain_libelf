...
static void print_syms(const char *shname, const char *strtab, ElfW(Sym) *syms, size_t entries)
{
...
int main(int argc, char *argv[])
{
	...
	const char *shname, *dynstr, *strtab;
...
//	[33] .shstrtab         STRTAB          0000000000000000 00404a 00013e 00      0   0  1
	...
	for (size_t i = 0; i < shnum; i++) {
		...
		if (!strcmp(".dynstr", shname)) {
			dynstr = file_mmbase + shdrs[i].sh_offset; 
		}
		if (!strcmp(".strtab", shname)) {
			strtab = file_mmbase + shdrs[i].sh_offset;
		}
	}
	...
	for (size_t i = 0; i < shnum; i++) {
		...
		if (shdr->sh_type == SHT_SYMTAB || shdr->sh_type == SHT_DYNSYM) {
			...
			print_syms(shname, !strcmp(shname, ".dynsym") ? dynstr : strtab, syms, entries);	
		}
	}
